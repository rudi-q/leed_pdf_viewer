<script lang="ts">
  ...
  onMount(() => {
    ...
    // Check if we should create a blank PDF from URL parameter
    if (browser && $page && $page.url) {
      const createBlank = $page.url.searchParams.get('create-blank');
      if (createBlank === 'true') {
        console.log('[Create Blank] Triggering blank PDF creation from menu');
        // Remove the parameter and create blank PDF
        goto('/', { replaceState: true });
        setTimeout(() => handleCreateBlankPDF(), 200);
      }
    }
    ...
    const handleMenuEvent = (event: Event) => {
      ...
      switch(event.type) {
        ...
        case 'menu-export-as-docx':
          ...
          break;
        case 'menu-share-pdf':
          // Note: Share is not available on the home page (no PDF loaded yet)
          console.log('Share PDF requested - but no PDF viewer on home page');
          break;
