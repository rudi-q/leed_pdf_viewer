<script lang="ts">
	...
	async function generateThumbnail(pageNumber: number): Promise<void> {
		...
		try {
			...
			const viewport = page.getViewport({ scale: 0.3, rotation: $pdfState.rotation }); // Small scale for thumbnails
			...
			const scaledViewport = page.getViewport({ scale: scale * 0.3, rotation: $pdfState.rotation });
