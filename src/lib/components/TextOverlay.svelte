<script lang="ts">
	...
	function handleOverlayClick(event: MouseEvent) {
		...
		// Inverse transform to 0-rotation space
		...
		// Store as relative position (0-1 range) - relative to true base dimensions
		const relativeX = basePageWidth > 0 ? baseX / basePageWidth : 0;
		const relativeY = basePageHeight > 0 ? baseY / basePageHeight : 0;
	...
	function saveEdit() {
		...
		if (newText === '') {
			...
			updateTextAnnotation({
				...
				relativeWidth:
					basePageWidth > 0 ? clampedWidth / basePageWidth : editingAnnotation.relativeWidth,
				relativeHeight:
					basePageHeight > 0 ? clampedHeight / basePageHeight : editingAnnotation.relativeHeight
			});
		}
	...
	function getDisplayPosition(annotation: TextAnnotation) {
		...
		const safeScale = getSafeScale();
		const baseX = annotation.x !== undefined ? annotation.x : annotation.relativeX * basePageWidth;
		const baseY = annotation.y !== undefined ? annotation.y : annotation.relativeY * basePageHeight;

		// Transform to current rotation
		...
		return {
			x: rotatedPoint.x * safeScale,
			y: rotatedPoint.y * safeScale
		};
	}
	...
	function updatePosition(annotation: TextAnnotation, displayX: number, displayY: number) {
		// Convert from display coordinates to rotated base scale
		const safeScale = getSafeScale();
		const rotatedBaseX = displayX / safeScale;
		const rotatedBaseY = displayY / safeScale;

		// Inverse transform to 0-rotation space for storage
		const basePoint = inverseTransformPoint(
			rotatedBaseX,
			rotatedBaseY,
			rotation as RotationAngle,
		...
		const relativeX = basePageWidth > 0 ? baseX / basePageWidth : annotation.relativeX;
		const relativeY = basePageHeight > 0 ? baseY / basePageHeight : annotation.relativeY;

		updateTextAnnotation({
			...annotation,
			x: baseX, // Store at base scale 0-rotation
			y: baseY, // Store at base scale 0-rotation
			relativeX,
			relativeY
		});
	}
	...
	function handleMouseMove(event: MouseEvent) {
		if (draggedAnnotation) {
			...
			const newX = Math.max(-500, Math.min(canvasWidth + 500, annotationStart.x + deltaX));
			const newY = Math.max(-500, Math.min(canvasHeight + 500, annotationStart.y + deltaY));
			...
			const newFontSize = Math.max(
				minFontSize,
				Math.min(widthBasedSize, heightConstraint, maxFontSize)
			);
			...
			updateTextAnnotation({
				...
				relativeX: basePageWidth > 0 ? newX / basePageWidth : resizingAnnotation.relativeX,
				relativeY: basePageHeight > 0 ? newY / basePageHeight : resizingAnnotation.relativeY,
				relativeWidth:
					basePageWidth > 0 ? newWidth / basePageWidth : (resizingAnnotation.relativeWidth ?? 0),
				relativeHeight:
					basePageHeight > 0 ? newHeight / basePageHeight : (resizingAnnotation.relativeHeight ?? 0)
			});
		}
	}
		...
		{#if editingAnnotation && editingAnnotation.id === annotation.id}
			...
			>
				<textarea
					...
					style="font-size: {annotation.fontSize *
						currentScale}px; color: {annotation.color}; font-family: {annotation.fontFamily};"
