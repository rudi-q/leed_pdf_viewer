		...
		try {
			...
			reader.onload = () => {
				try {
					...
					reject(
						new Error(
							`Failed to extract base64 data: ${error instanceof Error ? error.message : 'Unknown error'}`
						)
					);
				}
			};
			...
			reject(
				new Error(
					`Failed to create blob for base64 conversion: ${error instanceof Error ? error.message : 'Unknown error'}`
				)
			);
		}
	});
...
export async function exportPDFToDocx(
	...
	try {
		...
		try {
			response = await fetch('https://68d0f8e2003044d7955f.fra.appwrite.run/', {
				method: 'POST',
				headers: {
