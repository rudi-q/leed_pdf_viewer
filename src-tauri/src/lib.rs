    ...
    .setup(|app| {
      ...
      if args.len() > 1 {
        ...
        if !pdf_files.is_empty() {
          ...
          thread::spawn(move || {
            for attempt in 1..=20 {  // Try for up to 20 attempts (10 seconds)
              ...
              for (i, pdf_file) in pdf_files.iter().enumerate() {
                let event_name = if i == 0 { "file-opened" } else { "additional-file-opened" };

                match app_handle.emit(event_name, pdf_file) {
                  Ok(_) => {
                    println!("Attempt {}: Successfully emitted {} for {}", attempt, event_name, pdf_file);
              ...
              // Emit debug info
              let _ = app_handle.emit("debug-info", format!("File loading attempt {} of 20", attempt));
            }
